            ...
            for name, param in partial_state.items():
                if (is_qkv_weight(name) or "o_proj" in name) and args.kv_size_multiplier > 1:
                    ...
                    if "k" in name or "v" in name:
                        ...
                        full_model[name] = torch.transpose(full_weight, 0, 1).detach(
                        ).clone() if "o_proj" in name else full_weight.detach().clone()
    ...
    for name, full_p in full_state.items():
        ...
        if (is_qkv_weight(name) or "o_proj" in name) and kv_size_multiplier > 1:
            ...
            if updated_name in partial_state:
                if token == "gate_proj":
                    partial_state[updated_name] = torch.cat(
                        [to_load, partial_state[updated_name]], dim=0).detach().clone()
                else:
                    partial_state[updated_name] = torch.cat(
                        [partial_state[updated_name], to_load], dim=0).detach().clone()
            else:
