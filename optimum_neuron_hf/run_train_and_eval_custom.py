...
class DSKTrainer(NeuronTrainer):
    ...
    #     # print(f"DEBUG LOSS FOR ME PLEASE: {model.device=}, {loss=}")
    #     print(f"DEBUG LOSS FOR ME PLEASE: {model.device=}, {type(loss)=} {loss.dtype=} {loss.shape} {loss.device}")
    #     return loss
    
    #     output (valid):
    #     DEBUG WHAT IS WHERE: model.device=device(type='xla', index=1), xm.xla_device()=device(type='xla', index=1) [('input_ids', device(type='xla', index=1)), ('labels', device(type='xla', index=1))]
    #     DEBUG WHAT IS WHERE: model.device=device(type='xla', index=0), xm.xla_device()=device(type='xla', index=0) [('input_ids', device(type='xla', index=0)), ('labels', device(type='xla', index=0))]
    #     DEBUG LOSS FOR ME PLEASE: model.device=device(type='xla', index=0), loss=tensor(0.0144, device='xla:0', grad_fn=<SimpleCrossEntropyLossBackward>)
    #     DEBUG LOSS FOR ME PLEASE: model.device=device(type='xla', index=1), loss=tensor(0.0144, device='xla:1', grad_fn=<SimpleCrossEntropyLossBackward>)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # loss = DSKTrainer._masked_loss(loss_fct, logits, labels.to(logits.device), model.config.vocab_size)
